  ...
  describe(`"checkFieldExist" fn`, () => {
    describe("Exception Path", () => {
      it("Passing array as an input, should throw an error", () => {
        expect(() =>
          objectValidation.checkFieldExist([{ test: "value" }], "someField")
  ...
  describe(`"allowUndefinedField" fn`, () => {
    describe("Happy Path", () => {
      it("Passing undefined as an input, should return true", () => {
        expect(objectValidation.allowUndefinedField(undefined)).toBe(true);
      });
      ...
      it("Passing other than undefined as an input, should return false", () => {
        ...
        expect(objectValidation.allowUndefinedField(true)).toBe(false);
        expect(objectValidation.allowUndefinedField({})).toBe(false);
        expect(objectValidation.allowUndefinedField([])).toBe(false);
        expect(objectValidation.allowUndefinedField("")).toBe(false);
        expect(objectValidation.allowUndefinedField(0)).toBe(false);
        expect(objectValidation.allowUndefinedField(false)).toBe(false);
      });
    });
  });
  ...
  describe(`"allowFields" fn`, () => {
    describe("Exception Path", () => {
      ...
      it("Passing array as an input data, should throw an error", () => {
        expect(() => objectValidation.allowFields([] as any, [])).toThrow(
          GenericError
        );
        expect(() => objectValidation.allowFields([] as any, [])).toThrow(
      ...
      it("Passing other than array of strings for fields input, should throw an error", () => {
        ...
        expect(() =>
          objectValidation.allowFields({}, [true as any, false as any])
        ).toThrow("Fields input is invalid, expected an array of strings");
        expect(() =>
          objectValidation.allowFields({}, [true as any, false as any])
        ).toThrow("Fields input is invalid, expected an array of strings");
    ...
    describe("Happy Path", () => {
      ...
      it("Passing extra fields to an input data, should return false", () => {
        expect(
          objectValidation.allowFields({ car: "honda", bike: "Hero" }, ["car"])
        ).toBe(false);
      });
    });
  });
